...
void callFunction(char const *name, int argsCount) {
	if(currentFunction != NULL) {
		addFunctionReturnAddress(instructionsCount() + 1);
	}

	int stackSize = getStackSize();
	allocTemp(), allocTemp();
	assemblyOutput(COP" %d 1 ; Sauvegarde adresse retour %s", RETURN_ADDRESS, currentFunction ? currentFunction->name : "NULL"); // Sauvegarde adresse de retour
	...
	assemblyOutput(AFC" %d %d ; taille de la pile", STACK_SIZE_ADDRESS, stackSize);
	assemblyOutput(ADD" 0 0 %d ; Incrémentation stack pointer", STACK_SIZE_ADDRESS); // Incrémentation stack pointer
